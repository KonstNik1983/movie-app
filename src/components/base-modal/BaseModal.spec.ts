...
describe('BaseModal.vue', () => {
  it('рендерится только когда isShow = true', async () => {
    ...
    expect(wrapper.find('[data-testid="BaseModal_root"]').exists()).toBe(false);

    await wrapper.setProps({ isShow: true });
    expect(wrapper.find('[data-testid="BaseModal_root"]').exists()).toBe(true);
  });

  it('вызывает emit close при клике на overlay', async () => {
    const wrapper = mount(BaseModal, { props: { isShow: true } });

    await wrapper.find('[data-testid="BaseModal_overlay"]').trigger('click');
  ...
  it('рендерит слот внутри модалки', () => {
    ...
    expect(wrapper.find('[data-testid="BaseModal_content"] p').text()).toBe(
      'Hello'
    );
  });
});
