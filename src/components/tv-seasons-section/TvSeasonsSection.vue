<template>
  <div class="tv-seasons-section">
    <div class="season-tabs">
      ...
      <span v-if="extraSeasonsCount > 0" class="more-seasons"> + {{ extraSeasonsCount }} </span>
    </div>

    <div class="episodes-list" v-if="activeSeasonEpisodes?.episodes?.length">
      <div class="episode-card" v-for="episode in first4Episodes" :key="episode.id">
        <img
...
const filteredSeasons = computed(() =>
  props.seasons.filter((s) => s.season_number !== 0 && s.season_number != null),
);
...
const first4Episodes = computed(() => activeSeasonEpisodes.value?.episodes?.slice(0, 4) || []);

const extraEpisodesText = computed(() => {
  const total = activeSeasonEpisodes.value?.episodes?.length ?? 0;
  const extra = total - 4;
  if (extra > 0) return `+ ${pluralizeRu(extra, ["серия", "серии", "серий"])}`;
  return null;
});
</script>
