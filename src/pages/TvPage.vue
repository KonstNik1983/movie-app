<template>
  ...
  <LazySection>
    <component :is="AsyncMediaSimilar" :title="tv?.name" :movies="topSimilarTvFormatted" />
  </LazySection>
</template>
...
const { tv, credits, similar, seasonEpisodes, isLoading } = storeToRefs(tvStore);
...
const tvMeta = computed(() => {
  const parts: string[] = [];
  if (tv.value?.vote_average) parts.push(`⭐ ${tv.value.vote_average.toFixed(1)}`);
...
const creator = computed(() => tv.value?.created_by?.map((c) => c.name).join(", ") ?? null);
...
const tvCountries = computed(() => tv.value?.origin_country?.slice(0, 1).join(", ") ?? null);
...
const episodeDuration = computed(() => {
  ...
  const minutes = firstEpisode?.runtime ?? tv.value?.episode_run_time?.[0] ?? 0;
