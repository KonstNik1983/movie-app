...
import type { MovieGenreSection, MovieByGenre } from "@/store/movies/movies.types";
...
export const useMoviesStore = defineStore("moviesStore", () => {
  ...
  const formattedHomeMovieSections = computed(() => formattedMovieSections.value.slice(0, 3));
  ...
  const loadMovies = async () => {
    ...
    try {
      ...
      const promises = genres.map((genre) => discoverMovie({ with_genres: String(genre.id) }));
